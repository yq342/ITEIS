...
def Yq_Excat_Blat_Res(input_Group,input_Num):
    yq_columns = ["match","mismatch","rep.match","N_s","Q_gap_count","Q_gap_bases","T_gap_count","T_gap_bases","strand","Q_name","Q_size",
                             "Q_start","Q_end","T_name","T_size","T_start","T_end","block_count","blockSizes","qStarts","tStarts"]
...
while line1:
    ...
    Goal_Blat_res_1.to_csv("./" + Group + "/" + Group + "_All_TE_blat_Goal_seq_1_output",sep='\t',header=True,index=None)
    Goal_Blat_res_2.to_csv("./" + Group + "/" + Group + "_All_TE_blat_Goal_seq_2_output",sep='\t',header=True,index=None)
    line1 = f1.readline()
f1.close()
...
while line1:
    ...
    Goal_Read_Name_and_Goal_Site_TJ = pd.read_csv("./" + Group + "/" + Group + "_Read_Name_and_Goal_Site_TJ",sep='\t',header=0,index_col=None)
    Goal_Blat_res_1 = pd.read_csv("./" + Group + "/" + Group + "_All_TE_blat_Goal_seq_1_output",sep='\t',header=0,index_col=None)
    Goal_Blat_res_2 = pd.read_csv("./" + Group + "/" + Group + "_All_TE_blat_Goal_seq_2_output",sep='\t',header=0,index_col=None)



    # Goal_Blat_res_1_eff_Map_TE_Right = Goal_Blat_res_1[(Goal_Blat_res_1["Q_start"]<=10)&(Goal_Blat_res_1["Q_end"]<=140)&(Goal_Blat_res_1["T_end"]>=Goal_Blat_res_1["T_size"]-10)]
    # Goal_Blat_res_1_eff_Map_TE_Left = Goal_Blat_res_1[(Goal_Blat_res_1["Q_start"]>=10)&(Goal_Blat_res_1["Q_end"]>=140)&(Goal_Blat_res_1["T_start"]<=10)]


    # Goal_Blat_res_2_eff_Map_TE_Right = Goal_Blat_res_2[(Goal_Blat_res_2["Q_start"]<=10)&(Goal_Blat_res_2["Q_end"]<=140)&(Goal_Blat_res_2["T_end"]>=Goal_Blat_res_2["T_size"]-10)]
    # Goal_Blat_res_2_eff_Map_TE_Left = Goal_Blat_res_2[(Goal_Blat_res_2["Q_start"]>=10)&(Goal_Blat_res_2["Q_end"]>=140)&(Goal_Blat_res_2["T_start"]<=10)]
    ...
    All_TE_Seq_Map_Q_Sort.to_csv("./" + Group + "/" + Group + "_Goal_Blat_res_eff_Map_TE.csv",sep=',',header=True,index=None)
